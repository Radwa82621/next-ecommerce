...
const NavItem = ({ href, children, badge }: { href: string, children: React.ReactNode, badge?: number }) => (
...
export default function Navbar() {
  ...
  // Safe access to wishlist info to prevent hydration mismatch if it differs
          ...
          {status === 'authenticated' && (
            <div className="hidden lg:flex items-center gap-8">
              <ul className="flex items-center space-x-8 text-sm font-medium text-muted-foreground">
                ...
                <li><NavItem href="/wishlist" badge={wishlistCount}>Wishlist</NavItem></li>
          ...
          <div className="flex items-center gap-4">
            ...
            {status === "authenticated" ? (
              <button 
                ...
                className="hidden lg:flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-destructive transition-colors cursor-pointer
"
              >
            ...
            {status === 'authenticated' ? (
              <ul className="flex flex-col space-y-4 font-medium text-muted-foreground text-center">
                ...
                <li className="flex justify-center">
                  <NavItem href="/wishlist" badge={wishlistCount}>Wishlist</NavItem>
                </li>
                <li className="flex justify-center">
                  <NavItem href="/cart" badge={NumberOfCartItems}>Cart</NavItem>
                </li>
                <li className="pt-2 border-t cursor-pointer">
                  <button onClick={() => { handleLogOut(); setIsOpen(false); }} className="text-destructive hover:text-destructive/80 cursor-pointer
">
                    Sign Out
                  </button>
                </li>
